...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		boolean isMember = Util.computeLogic(key, node.getNodeID().add(new BigInteger("1")), stub.getNodeID());
		
		// if logic returns true, then return the successor
		if(isMember) {
			return successor;
		} else {
			// else; call findHighestPredecessor(key)
			...
			return highest_pred.getSuccessor();
		}
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for(int i = fingerTable.size()-1; i >= 0; i--) {
			...
			NodeInterface fingStub = Util.getProcessStub(finger.getNodeName(), finger.getPort());
			...
			boolean isMember = Util.computeLogic(fingStub.getNodeID(), node.getNodeID().add(oneInt), key.subtract(oneInt));
