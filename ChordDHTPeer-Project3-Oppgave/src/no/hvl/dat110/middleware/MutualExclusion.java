...
public class MutualExclusion {
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for (Message node : activenodes) {
			...
			NodeInterface nodeStub = Util.getProcessStub(node.getNodeIP(), node.getPort());
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		case 2: {
			...
			int receivedMsgClock = message.getClock();

			// own clock for the multicast message
			int ownMsgClock = node.getMessage().getClock();

			// in case of draw ( if comp is less than zero we know that message nodeID is lowest )
			int comp = message.getNodeID().compareTo(node.getNodeID());

			// compare clocks, the lowest wins
			// if clocks are the same, compare nodeIDs, the lowest wins
			if (receivedMsgClock < ownMsgClock || (receivedMsgClock == ownMsgClock && comp < 0)) {
	...
	/**
	 * Multicasts release locks message to other processes including self
	 *
	 * @param activenodes
	 * @throws RemoteException
	 */
	...
	/**
	 * Checks that all replicas have replied
	 *
	 * @param numvoters
	 * @return boolean
	 * @throws RemoteException
	...
	private boolean areAllMessagesReturned(int numvoters) throws RemoteException {
		...
		boolean checkSizes = queueack.size() == numvoters;
